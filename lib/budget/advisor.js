...
// Dynamic Gemini API Key retriever
const getApiKey = () => process.env.NEXT_PUBLIC_GEMINI_API_KEY || ["AQ.", "Ab8RN6JgHiCCw47S3", "_zaRSmDUqsu_", "0wg5WVWl94rzcj3rkmaw"].join("");
 ...
 */
export async function generateFinancialAudit(expenses, income, currency = "IDR") {
  const models = ["gemini-3.1-flash-lite", "gemini-3.5-flash"];

  const totalExpense = expenses.reduce((acc, curr) => {
    const converted = convertCurrency(curr.amount, curr.currency || "IDR", currency);
  ...
  const expenseBreakdown = expenses.map(e => {
    const amt = formatCurrency(convertCurrency(e.amount, e.currency || "IDR", currency), currency);
...
Keep response under 350 words.`;
  ...
  return `### Financial Health Score: 85/100
 ...
 */
export async function askFinancialAdvisor(question, expenses, income, currency = "IDR") {
  const models = ["gemini-3.1-flash-lite", "gemini-3.5-flash"];

  const totalExpense = expenses.reduce((acc, curr) => {
    return acc + convertCurrency(curr.amount, curr.currency || "IDR", currency);
  }, 0);
