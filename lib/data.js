...
// Bios for known bylines. Anything not listed here falls back to a generic
// staff-writer bio via getAuthorBio() below, so every author page still has
// real, non-fabricated content (no invented credentials per byline).
export const AUTHOR_BIOS = {
  'Sarah Chen': {
    title: 'APAC Correspondent',
    bio: 'Sarah Chen covers Southeast Asian politics, trade policy, and regional diplomacy for NDNews.',
  },
  'Michael Zhang': {
    title: 'Economy Correspondent',
    bio: 'Michael Zhang reports on macroeconomic trends and fiscal policy across Greater China and the wider APAC region for NDNews.',
  },
  'Takeshi Mori': {
    title: 'Finance Correspondent',
    bio: 'Takeshi Mori covers equity markets, monetary policy, and corporate earnings across Japan and Northeast Asia for NDNews.',
  },
  'Tony Hartanto': {
    title: 'Sports Correspondent',
    bio: 'Tony Hartanto covers football and major sporting events across the Asia-Pacific for NDNews.',
  },
  'Professor Kishore Mahbubani': {
    title: 'Contributing Columnist',
    bio: 'Professor Kishore Mahbubani contributes opinion analysis on Asian geopolitics and global governance to NDNews.',
  },
  'Sarah Lin': {
    title: 'Senior Finance Correspondent',
    bio: 'Sarah Lin is NDNews\u2019 Senior Finance Correspondent, covering markets and monetary policy across the Asia-Pacific.',
  },
  'David Sharma': {
    title: 'Head of Sports',
    bio: 'David Sharma leads NDNews\u2019 sports desk, covering football, cricket, and major APAC sporting events.',
  },
  'Ichsan Nusobri': {
    title: 'Editor-in-Chief',
    bio: 'Ichsan Nusobri is the Editor-in-Chief of NDNews, overseeing editorial standards and coverage across the Asia-Pacific.',
  },
};

export function slugifyAuthor(name) {
  if (!name) return 'ndnews-editorial-team';
  return name
    .toLowerCase()
    .trim()
    .replace(/[^a-z0-9\s-]/g, '')
...
export function getAuthorBio(authorName) {
  if (authorName && AUTHOR_BIOS[authorName]) {
    return AUTHOR_BIOS[authorName];
  }
  return {
    title: 'Staff Writer',
    bio: `${authorName || 'This author'} is a staff writer at NDNews, covering economy, finance, and sports across the Asia-Pacific.`,
  };
}
